Arbiter.Validator = (function () {
    return {
        ...
        startValidation: function (attributeJsonArr, qaOptionJsonArr, layerNameArr, FIDArr) {
            ...
            Arbiter.Validator.sendObjectRequest("http://175.116.181.51:8080/opengds/validator/validate.ajax", validationGeoJson, Arbiter.Validator.doneCallback);
        },

        // 확장된 아비터 검수 함수
        start: function (attributeJsonArr) {
            Android.StartValidationProgressDialog();
            console.log("validationGeoJson : " + JSON.stringify(attributeJsonArr));
            // Send to Validator Server
            Arbiter.Validator.sendObjectRequest("http://175.116.181.51:8080/geodt/mobile/validate.do", attributeJsonArr, Arbiter.Validator.doneCallback);
        },
        ...
        processDone: function (data, textStatus, jqXHR, doneCallback) {
            if (data.features.length == 0) {
                ...
                Android.SaveValidationResult("noError", true);
            } else {
        ...
        resultErrorMarking: function (result) {
            ...
            for (var i = 0; i < errorResults.length; i++) {
                ...
                markingPoints[i].attributes = {featureID: errorResults[i].properties.featureID};
            ...
            if (layer == null) {
                layer = new OpenLayers.Layer.Vector('ErrorMarking-wfs', {
...
//           var layer = Arbiter.Layers.getLayerById(layerId, Arbiter.Layers.type.WFS);
            var layer = Arbiter.Layers.getLayerById('ErrorMarking', Arbiter.Layers.type.WFS);
            var feature;/* = layer.getFeatureByFid(fid)*/
            ;
            for (var i = 0; i < layer.features.length; i++) {
                if (layer.features[i].attributes.featureID == fid) {
