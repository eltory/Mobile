...
import com.lmn.OpenGDS_Android.ListAdapters.ValidationAdapter.ValidationOptionSettingListAdapter;
...
public class ValidationOptionSettingDialog extends ArbiterDialogFragment {
    ...
    public void onPositiveClick() {
        (new ArbiterDialogs(getActivity().getApplicationContext(), getActivity().getResources(),
                getActivity().getSupportFragmentManager())).showLayersDialog(hasThreadPool);
        getDialog().dismiss();
    }
    ...
    private void populateLayersList(View view) {
        ...
        if (selectedLayer.getFeatureGeometryType().equalsIgnoreCase("MULTIPOLYGON") ||
                selectedLayer.getFeatureGeometryType().equalsIgnoreCase("POLYGON")) {
            String[] polygonArr = selectedLayer.getPolygonOptionNames();
            validationOptionSettingAdapter.setData(polygonArr);
        } else if (selectedLayer.getFeatureGeometryType().equalsIgnoreCase("MULTILINESTRING") ||
                selectedLayer.getFeatureGeometryType().equalsIgnoreCase("LINESTRING")) {
